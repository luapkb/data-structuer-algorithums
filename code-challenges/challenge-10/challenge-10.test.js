...
const count = (target, input) => {
  let timmy =0;
  arr.forEach((input) => (input === value && timmy++));
  return timmy;
};
...
const totalSum = (input) => {
  let timmy = imput.reduce((sum, arr) => {
    sum += arr.reduce((accum, value) => {
      accum += value;
    }, 0)
    return sum;
  }, 0)
...
const divisibleByFiveTwoToThePower = (input) => {
  let timmy = input.map(arr => {
    let timmyChair = arr.filter(num => {
      if(num % 5 === 0 && type of num === 'number'){
        return true;
      }
    })
    let timmyHair = timmyChair.map(value => {
